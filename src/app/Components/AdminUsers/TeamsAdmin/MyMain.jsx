...
const MyMain = async ({ part1, name }) => {
  ...
  const response = await axios.get(
    `${process.env.NEXT_PUBLIC_Backend_URL}api/v1/Users/AllUsers?role=admin&role=manager`,
    config
  );
  ...
  // console.log("response================");
   newItems = response.data.data.map((item, index) => {
       return {
         ...
         phone: item.phone,
         image:
           item.profileImage && item.profileImage != "" ? item.profileImage.url : "",
          type : item.role,
         actions: "Delete",
       };
  ...
  return (
    <Box
      sx={{ mt: { xs: "5px", sm: 13,md : 6 }, pb: 6, bgcolor: "#ECECEC" }}
    >
