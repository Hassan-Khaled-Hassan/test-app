...
const MyFormContainer = ({ part1, name, newItems }) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      headerAlign: "center",
      headerClassName: "super-app-theme--header",
      renderCell: ({ row: { registrarId, type } }) => (
        <Box
          width="99px"
          m="7px auto"
          p="5px"
        ...
        >
          ...
          {type === "user" && <LockOpenIcon sx={{ color: "white" }} />}
          <Typography sx={{ mt: "3px", fontSize: "15px", color: "white" , textTransform : "capitalize" }}>
            {type}
          </Typography>
        </Box>
      ),
    },
    {
      ...
      renderCell: ({ row: { registrarId } }) => (
        <Box display="flex" justifyContent="center" alignItems="center">
          <Link
            href={`/AdminDashboard/admin/Users/users/${registrarId}`}
            passHref
          >
  ...
  return (
    ...
    >
      <MyHeaderThree
        part1={part1}
        name={name}
        paragraphs={["This page displays all Users"]}
      />
