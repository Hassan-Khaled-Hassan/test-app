...
import { AdminDeleteCertificate } from "@/app/lib/Slices/CertificateSlice";
...
const useCategoriesHook = () => {
  ...
  const handleDeleteClick = async (id) => {
    // console.log(id);
    setLoading(true);
    await dispatch(AdminDeleteCertificate(id)).unwrap();
    setLoading(false);
  };
  const { AdminDeleteCertificates, error } = useSelector(
    (state) => state.Certificates
  );
  // console.log(AdminDeleteCertificates);
  // console.log(error);
  ...
  useEffect(() => {
    if (error) {
      ...
      AdminDeleteCertificates.status === 200
    ) {
